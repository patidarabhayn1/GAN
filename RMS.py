...
# %%
class DCTrainer():
    def __init__(self,model , dataloader , optim_gen , optim_critic  , epochs, device ,latent_dims, lambda_ = 5 , path ='RMSprop'):
    ...
    def show(self , nx = 10 , e = 0 , images = None):
        ...
        plt.show()
        plt.savefig(f"{self.path}/images/generated_{e}.png")
    ...
    def resume(self , path , iterations):
        ...
        self.optim_critc.learning_rate = 1e-4
        self.optim_gen.learning_rate = 1e-3
...
trainer = DCTrainer(model , dataloader , optim_critic=optim_critc, optim_gen=optim_gen , epochs=epochs , device = device , latent_dims=input_dims  ,lambda_=15)

# trainer.resume(path , 1000)
trainer.train()
